...
#
if [[ ! -e ~/.config/roperdot/roperdot-help.txt ]]; then
    echo Run generate-help before running help to build ~/.config/roperdot/roperdot-help.txt
    exit 1
fi
...
Prints the contents of ~/.config/roperdot/roperdot-help.txt. If a pattern is
provided, only those lines that match will be shown.

To refresh the help file, run generate-help.
...
if [[ $# -eq 0 ]]; then
    cat ~/.config/roperdot/roperdot-help.txt
elif [[ "$1" == "-g" ]]; then
	cat <<EOT
...
EOT
    ...
    grep -i "$1" < ~/.config/roperdot/roperdot-help.txt
fi