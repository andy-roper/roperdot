#
# Description: Generates a SHA256 checksum for binary data
#
...
if [[ "$1" == "--help" || "$1" == "-h" || "$1" == "-?" ]]; then
	cat <<EOT
sha1: generate a sha256sum for binary data
...
piped input. If neither a string nor a filespec nor piped input are provided,
...
if [[ -n "$input_file" ]]; then
	if [[ -n "$write_to_clipboard" ]]; then
		sha256sum -b "$input_file" | clipcopy
	else
		sha256sum -b "$input_file"
	fi
else
	if [[ -n "$str" ]]; then
		if [[ -n "$write_to_clipboard" ]]; then
			echo "$str" | sha256sum -b | clipcopy
		else
			echo "$str" | sha256sum -b
		fi
	else
